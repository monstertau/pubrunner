    ...
    for resourceGroupName in toolSettings["resources"]:
        ...
        for resource in toolSettings["resources"][resourceGroupName]:
            if isinstance(resource, dict):
                assert len(
                    resource.items()) == 1, "ERROR in pubrunner.yml: A resource (%s) is not being parsed correctly. It is likely that the resource settings (e.g. format) are not indented properly. Try indenting more" % (
                    str(list(resource.keys())[0]))
...
def chunk_pmid_list(pmid_list, n=200):
...
def prepareConversionAndHashingRuns(toolSettings, mode, workingDirectory, get_resources=True):
    ...
    for resourceGroupName in ["all", mode]:
        for resName, projectSettings in toolSettings["resources"][resourceGroupName]:
            allowed = ['pmid_filepath', 'rename', 'format', 'removePMCOADuplicates', 'usePubmedHashes', 'pmids',
                       'pmcids']
            ...
            if resName == 'PUBMED_CUSTOM':
                assert 'pmids' or 'pmid_filepath' in projectSettings, 'Must provide pmids or pmid_filepath when using PUBMED_CUSTOM ' \
                                                                      'resource'
            ...
            if "format" in projectSettings:
                ...
                conversionInfo = (
                    os.path.join(workingDirectory, inDir), inFormat, os.path.join(workingDirectory, outDir), outFormat,
                    chunkSize)
                conversionInfo = {}
                ...
                if "generatePubmedHashes" in resInfo and resInfo["generatePubmedHashes"] == True:
                    ...
                    hashesInfo = {'resourceDir': os.path.join(workingDirectory, inDir), 'hashDir': hashesSymlink,
                                  'removePMCOADuplicates': removePMCOADuplicates, 'whichHashes': whichHashes}
...
def cleanWorkingDirectory(directory, doTest, doCustom=False, execute=False):
    mode = "test" if doTest else "full"
    mode = "custom" if doCustom else mode
    globalSettings = pubrunner.getGlobalSettings()
    os.chdir(directory)
...
def pubrun(directory, doTest, doCustom, get_resources=True, forceresource_dir=None, forceresource_format=None,
           outputdir=None):
    mode = "test" if doTest else "full"
    mode = "custom" if doCustom else mode
    ...
    if os.path.isdir('.pubrunner_lock'):
        raise RuntimeError(
            "A .pubrunner_lock directory exists in this project directory. These are created by PubRunner during an incomplete run. Are you sure another instance of PubRunner is not currently running? If you're sure, you will need to delete this directory before continuing. The directory is: %s" % os.path.join(
                directory, '.pubrunner_lock'))
        ...
        for conversionInfo in toolSettings["conversions"]:
            ...
            if useHashes:
                ...
                assert os.path.isdir(
                    pmidDirectory), "Cannot find PMIDs directory for resource. Tried: %s" % pmidDirectory
